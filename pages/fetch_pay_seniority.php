...
if (isset($_POST['pay_seniority_job_id']) && !empty($_POST['pay_seniority_job_id'])) {
    $query = 'SELECT * FROM employee_registration
        ...
        WHERE employee_registration.er_id = "'. $_POST['pay_seniority_job_id'] .'"';
    ...
    if (mysqli_num_rows($result)>0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <input type="text" class="form-control" id="txt_er_salary" name="txt_er_salary" value="' . $row["er_salary"] . '" readonly />
            </div>';
            echo'<div class="form-group col-xs-12">
            <label>Tax Salary:</label>
            <input type="text" class="form-control" id="txt_er_salary_tax" name="txt_er_salary_tax" value="' . $row["er_salary_tax"] . '" readonly />
            </div>';
        }
    }
}
