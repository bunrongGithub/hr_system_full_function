...
if (isset($_POST['user_job_id']) && !empty($_POST['user_job_id'])) {
   $query = 'SELECT * FROM employee_registration
        ...
        WHERE employee_registration.er_id = "' . $_POST['user_job_id'] . '"';
   $result = mysqli_query($connect, $query);
   if (mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
         echo '<div class="form-group col-xs-12">
            <label>Employee Name:</label>
            <input type="text" class="form-control" id="txt_er_name_kh" name="txt_er_name_kh" value="' . $row["er_name_kh"] . '" readonly />
            </div>';
         echo '<div class="form-group col-xs-12">
