...
if (isset($_POST["btnadd"])) {
   $v_company_group = $_POST["txt_company_group"];
   $v_get_id = $_GET['id'];

   $sql = "INSERT INTO  user_auth_company_group  
                        ( uacg_com_group_id 
                        , uacg_user_id
                        ) 
                  VALUES 
   ...
   header("location:user_com_group_sub.php?id=$v_get_id");
}
...
if (isset($_GET["del_id"])) {
   ...
   $sql = "DELETE FROM user_auth_company_group WHERE uacg_id = '$del_id'";
   $result = mysqli_query($connect, $sql);
   header("location:user_com_group_sub.php?id=$v_get_id");
}
                                    ...
                                    <div class="col-md-12">
                                       <form method="post" enctype="multipart/form-data" action="">
                                          ...
                                          <div class="form-group col-xs-6">
                                             ...
                                             <select class="form-control" id="txt_company_group" name="txt_company_group">
                                                <?php
                                                $sql = 'SELECT * FROM company_group';
                                                $result = mysqli_query($connect, $sql);
                        ...
                        <div class="box-body table-responsive">
                           <table id="info_data" class="table table-bordered table-striped">
                              ...
                              <tbody>
                                 <?php
                                 $set_id = $_GET["id"];
                                 $sql = "SELECT * FROM user_auth_company_group A
                                                        LEFT JOIN user B ON B.id=A.uacg_user_id
                                                        LEFT JOIN company_group C ON C.cg_id=A.uacg_com_group_id
                                                        WHERE uacg_user_id=$set_id
                                                        ";
                                       ...
                                       <td style="text-align:center;">
                                          ...
                                          <!-- <a onclick="doUpdate(<?php echo $row['sb_id']; ?>,
                                                        '<?php echo $v_name; ?>')" data-toggle="modal" data-target="#myModal_update" class="btn btn-primary btn-sm"><i style="color:white;" class="fa fa-edit"></i></a>
                                                         -->
                                          <a onclick="return confirm('Are you sure to delete ?');" href="user_com_group_sub.php?del_id=<?php echo $row['uacg_id']; ?>&sent_user_id=<?php echo $row['uacg_user_id']; ?>" class="btn btn-danger btn-sm"><i style="color:white;" class="fa fa-trash "></i></a>
                                       </td>
                                    </tr>
                                 <?php
                                 }
