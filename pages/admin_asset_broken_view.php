...
if (isset($_GET['view_id'])) {
   ...
   $_code = $row['adassb_code'];
   $_type = $row['adassb_type'];
   $_category = $row['adassb_category'];
   $_as_name = $row['adassb_asset_name'];
   $_as_qty = $row['adassb_qty'];
   $_as_mou = $row['adassb_mou'];
   $_as_total = $row['adassb_total'];
   $_as_broken_no = $row['adassb_broken_no'];
   $_broken_date = $row['adassb_broken_date'];
   $_broken_by = $row['adassb_broken_by'];
   $_start_date = $row['adassb_date'];
   $_price = $row['adassb_unit_price'];
   $_reason = $row['adassb_reason'];
   $_status = $row['adassb_status'];
   $_img = $row['adassb_img'];
}
if (isset($_GET['id'])) {
   $id_del = $_GET['id'];
   ...
   $result = mysqli_query($connect, $sql);
   header("location:admin_asset_broken_view.php?view_id=".$id."");
   exit();
}
?>
<html>
                        ...
                        <div class="row col-xs-12">
                           ...
                           <div class="row col-xs-4">
                              <div class="form-group col-xs-12">
                                 ...
                                 <input style="font-weight: 800;" type="text" value="<?= $_as_broken_no ?>" name="" id="" class="form-control">
                              </div>
                              <div class="form-group col-xs-12">
                                 <label for="">Broken Date:</label>
                                 <input style="font-weight: 800;" type="date" value="<?= $_broken_date ?>" name="" id="" class="form-control">
                              </div>
                              <div class="form-group col-xs-12">
                                 <label for="">Broken By Whom:</label>
                                 <input style="font-weight: 800;" type="text" value="<?= $_broken_by ?>" name="" id="" class="form-control">
                              </div>
                              <div class="form-group col-xs-12">
                                 <label for="">Strat Date:</label>
                                 <input style="font-weight: 800;" type="date" value="<?= $_start_date ?>" name="" id="" class="form-control">
                              </div>
                              <div class="form-group col-xs-12">
                                 <label for="">Current Unit Price:</label>
                                 <div class="input-group">
                                    <div class="input-group-addon">$</div>
                                    <input style="font-weight: 800;" type="text" name="" id="" value="<?= $_price . '$' ?>" class="form-control">
                                 </div>
                              </div>
                              <div class="form-group col-xs-12">
                           ...
                           <div class="row col-xs-10">
                              <table id='info_data' class="table table-striped table-responsive table-bordered">
                                 ...
                                 <tbody>
                                    <?php
                                    $sql = "SELECT * FROM admin_asset_broken_material A
                                                   LEFT JOIN text_asset_in_mou B ON B.aim_id = A.adsbm_mou
                                    WHERE adssbm_material_id = $id";
                                    $result = $connect->query($sql);
                                    $i=1;
                                    while ($row = $result->fetch_assoc()) {
                                       $_count_i = $i++;
                                       $_material_name = $row['adsbm_material_name'];
                                       $_mat_qty = $row['adsbm_qty'];
                                       $_mat_mou = $row['aim_name'];
                                       $_mat_remark = $row['adssbm_remark'];
                                    ?>
                                       <tr>
                                          <td class="text-center"><?=$_count_i;?></td>
                                          <td class="text-center"><?=$_material_name?></td>
                                          <td class="text-center"><?=$_mat_qty?></td>
                                          <td class="text-center"><?=$_mat_mou?></td>
                                          <td class="text-center"><?=$_mat_remark?></td>
                                          <td class="th_none_display text-center" style="width: 100px; vertical-align: middle; ">
                                             <a style="color: white;" class="btn btn_no_print btn-sm btn-danger" onclick="return confirm('Are you sure to delete?');" href="admin_asset_broken_view.php?id=<?= $row['adsbm_id'];?>&view_id=<?=$id?>"><i class="fa fa-trash"></i></a>
                                          </td>
                                       </tr>
                                    <?php
                                    }
