...
$targetDir = "../img/upload/name_card/";

if (isset($_POST["btnadd"])) {
    $v_company = $_POST["txt_company"];

    $sql = "INSERT INTO name_card 
                        ( nc_company_id ) 
                  VALUES 
                    ('$v_company')";
    $result = mysqli_query($connect, $sql);
    header('location:name_card.php?message=success');
}

if (isset($_POST["btnupdate"])) {
    $id = $_POST["namecard_id"];
    $v_company = $_POST["edit_company"];

    $sql = "UPDATE name_card SET nc_company_id = '$v_company' WHERE nc_id = $id";

    $result = mysqli_query($connect, $sql);
    header('location:name_card.php?message=update');
}
...
if (isset($_POST["btnimage"])) {
    ...
    if (!empty($_FILES['card_front']['name'])) {
        ...
        $sql = "UPDATE name_card SET nc_image_front = '$v_c_front' WHERE nc_id = $id";
        $result = mysqli_query($connect, $sql);
    }
    if (!empty($_FILES['card_back']['name'])) {
        ...
        $sql = "UPDATE name_card SET nc_image_back = '$v_c_back' WHERE nc_id = $id";
        $result = mysqli_query($connect, $sql);
    }
...
if (isset($_GET["del_id"])) {
    ...
    $sql = "DELETE FROM name_card WHERE nc_id = '$id'";
    $result = mysqli_query($connect, $sql);
    header("location: name_card.php?message=delete");
}
...
<body class="skin-black">
                            ...
                            <div class="box-header">
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <div class="col-md-12">
                                                    <form method="post" enctype="multipart/form-data" action="">
                                                        <input type="hidden" id="namecard_id" name="namecard_id" />
                                                        <div class="form-group col-xs-6">
                                                            <label>Company:</label>
                                ...
                                <div class="box-body table-responsive">
                                    <table id="info_data" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <?php
                                            $sql = "SELECT * FROM name_card LEFT JOIN company ON company.c_id = nc_company_id";
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                $v_card_front = $row["nc_image_front"];
                                                $v_card_back = $row["nc_image_back"];
                                            ?>
                                                <tr>
                                                    <td><?php echo $v_i; ?></td>
                                                    ...
                                                    <td style="text-align: center;">
                                                        <?php if ($v_card_front != '') {
                                                            echo '<img src="../img/upload/name_card/' . $v_card_front . '" width="120px" height="100px"/>';
                                                        } else {
                                                            echo '<img src="../img/no_image.jpg" width="120px" height="100px"/>';
                                                        } ?>
                                                        <a style="float:right; cursor:pointer;" onclick="doImage(<?php echo $row['nc_id']; ?>,
                                                    ...
                                                    <td style="text-align: center;">
                                                        <?php if ($v_card_back != '') {
                                                            echo '<img src="../img/upload/name_card/' . $v_card_back . '" width="120px" height="100px"/>';
                                                        } else {
                                                            echo '<img src="../img/no_image.jpg" width="120px" height="100px"/>';
                                                        } ?>
                                                        <a style="float:right; cursor:pointer;" onclick="doImage(<?php echo $row['nc_id']; ?>,
                                                    ...
                                                    <td style="text-align:center;">
                                                        <!-- <a href="edit_name_card.php?id=<?php echo $row['nc_id']; ?>" class="btn btn-info btn-sm"><i style="color:white;" class="fa fa-eye"></i></a> -->
                                                        <a onclick="doUpdate(<?php echo $row['nc_id']; ?>,
                                                        '<?php echo $row['nc_company_id']; ?>')" data-toggle="modal" data-target="#myModal_update" class="btn btn-primary btn-sm"><i style="color:white;" class="fa fa-edit"></i></a>
                                                        <a onclick="return confirm('Are you sure to delete ?');" href="name_card.php?del_id=<?php echo $row['nc_id']; ?>" class="btn btn-danger btn-sm"><i style="color:white;" class="fa fa-trash "></i></a>
                                                    </td>
                                                </tr>
                                            <?php
                                            }
    ...
    <script type="text/javascript">
        function doUpdate(id, companyid) {
            $('#namecard_id').val(id);
        ...
        function doImage(id, cardf, cardb) {
            ...
            if (cardf == '') {
                ...
                document.getElementById('show_front').setAttribute('src', '../img/upload/name_card/' + cardf);
                // $('#card_front').val(cardf);
            }
            if (cardb == '') {
                document.getElementById('show_back').setAttribute('src', '../img/no_image.jpg');
            } else {
                document.getElementById('show_back').setAttribute('src', '../img/upload/name_card/' + cardb);
                // $('#card_back').val(cardb);
            }
        }
        ...
        $(function() {
            ...
            $("#name_card").css("background-color", "##367fa9");
