...
if (isset($_POST["btnadd"])) {
    ...
    $sql = "INSERT INTO seniority 
                        ( sen_name ) 
                  VALUES 
                    ('$v_name')";
    $result = mysqli_query($connect, $sql);
    header('location:seniority.php?message=success');
}

if (isset($_POST["btnupdate"])) {
    $id = $_POST["seniority_id"];
    $v_name = $_POST["edit_name"];

    $sql = "UPDATE seniority SET sen_name = '$v_name' WHERE sen_id = $id";

    $result = mysqli_query($connect, $sql);
    header('location:seniority.php?message=update');
}

if (isset($_GET["del_id"])) {
    $id = $_GET["del_id"];

    $sql = "DELETE FROM seniority WHERE sen_id = '$id'";
    $result = mysqli_query($connect, $sql);
    header("location: seniority.php?message=delete");
}
...
<body class="skin-black">
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <div class="col-md-12">
                                                    <form method="post" enctype="multipart/form-data" action="">
                                                        <input type="hidden" id="seniority_id" name="seniority_id" />
                                                        <div class="form-group col-xs-6">
                                                            <label>Description:</label>
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td style="text-align:center;">
                                                        <!-- <a href="edit_overtime.php?id=<?php echo $row['sen_id']; ?>" class="btn btn-info btn-sm"><i style="color:white;" class="fa fa-eye"></i></a> -->
                                                        <a onclick="doUpdate(<?php echo $row['sen_id']; ?>,
                                                        '<?php echo $v_name; ?>')" 
                                                        data-toggle="modal" data-target="#myModal_update" class="btn btn-primary btn-sm"><i style="color:white;" class="fa fa-edit"></i></a>
                                                        <a onclick="return confirm('Are you sure to delete ?');" href="set_leave.php?del_id=<?php echo $row['sen_id']; ?>" class="btn btn-danger btn-sm"><i style="color:white;" class="fa fa-trash "></i></a>
                                                    </td>
                                                </tr>
                                            <?php
                                            }
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $("#seniority").addClass("active");
            $("#seniority").css("background-color", "##367fa9");
